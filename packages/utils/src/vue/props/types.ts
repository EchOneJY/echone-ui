import type { eoPropKey } from './runtime';
import type { IfNever, UnknownToNever, WritableArray } from './util';

import type { ExtractPropTypes, PropType } from 'vue';

type Value<T> = T[keyof T];

/**
 * Extract the type of a single prop
 *
 * 提取单个 prop 的参数类型
 *
 * @example
 * ExtractPropType<{ type: StringConstructor }> => string | undefined
 * ExtractPropType<{ type: StringConstructor, required: true }> => string
 * ExtractPropType<{ type: BooleanConstructor }> => boolean
 */
export type ExtractPropType<T extends object> = Value<
  ExtractPropTypes<{
    key: T;
  }>
>;

/**
 * Extracts types via `ExtractPropTypes`, accepting `PropType<T>`, `XXXConstructor`, `never`...
 *
 * 通过 `ExtractPropTypes` 提取类型，接受 `PropType<T>`、`XXXConstructor`、`never`...
 *
 * @example
 * ResolvePropType<BooleanConstructor> => boolean
 * ResolvePropType<PropType<T>> => T
 */
export type ResolvePropType<T> = IfNever<
  T,
  never,
  ExtractPropType<{
    required: true;
    type: WritableArray<T>;
  }>
>;

/**
 * Merge Type, Value, Validator types
 * 合并 Type、Value、Validator 的类型
 *
 * @example
 * EoPropMergeType<StringConstructor, '1', 1> =>  1 | "1" // ignores StringConstructor
 * EoPropMergeType<StringConstructor, never, number> =>  string | number
 */
export type EoPropMergeType<Type, Value, Validator> =
  | IfNever<UnknownToNever<Value>, ResolvePropType<Type>, never>
  | UnknownToNever<Validator>
  | UnknownToNever<Value>;

/**
 * Handling default values for input (constraints)
 *
 * 处理输入参数的默认值（约束）
 */
export type EoPropInputDefault<
  Required extends boolean,
  Default,
> = Required extends true
  ? never
  : Default extends Array<any> | Record<string, unknown>
    ? () => Default
    : (() => Default) | Default;

/**
 * Native prop types, e.g: `BooleanConstructor`, `StringConstructor`, `null`, `undefined`, etc.
 *
 * 原生 prop `类型，BooleanConstructor`、`StringConstructor`、`null`、`undefined` 等
 */
export type NativePropType =
  | ((...args: any) => any)
  | { new (...args: any): any }
  | null
  | undefined;
export type IfNativePropType<T, Y, N> = [T] extends [NativePropType] ? Y : N;

/**
 * input prop `buildProp` or `buildProps` (constraints)
 *
 * prop 输入参数（约束）
 *
 * @example
 * EoPropInput<StringConstructor, 'a', never, never, true>
 * ⬇️
 * {
    type?: StringConstructor | undefined;
    required?: true | undefined;
    values?: readonly "a"[] | undefined;
    validator?: ((val: any) => boolean) | ((val: any) => val is never) | undefined;
    default?: undefined;
  }
 */
export type EoPropInput<
  Type,
  Value,
  Validator,
  Default extends EoPropMergeType<Type, Value, Validator>,
  Required extends boolean,
> = {
  default?: EoPropInputDefault<Required, Default>;
  required?: Required;
  type?: Type;
  validator?: ((val: any) => boolean) | ((val: any) => val is Validator);
  values?: readonly Value[];
};

/**
 * output prop `buildProp` or `buildProps`.
 *
 * prop 输出参数。
 *
 * @example
 * EoProp<'a', 'b', true>
 * ⬇️
 * {
    readonly type: PropType<"a">;
    readonly required: true;
    readonly validator: ((val: unknown) => boolean) | undefined;
    readonly default: "b";
    __eoPropKey: true;
  }
 */
export type EoProp<Type, Default, Required> = {
  [eoPropKey]: true;
  readonly required: [Required] extends [true] ? true : false;
  readonly type: PropType<Type>;
  readonly validator: ((val: unknown) => boolean) | undefined;
} & IfNever<Default, unknown, { readonly default: Default }>;

/**
 * Determine if it is `EoProp`
 */
export type IfEoProp<T, Y, N> = T extends { [eoPropKey]: true } ? Y : N;

/**
 * Finalized conversion output
 *
 * 最终转换 EoProp
 */
export type EoPropFinalized<Type, Value, Validator, Default, Required> = EoProp<
  EoPropMergeType<Type, Value, Validator>,
  UnknownToNever<Default>,
  Required
>;

/**
 * Converting input to output.
 *
 * 将输入转换为输出
 */
export type EoPropConvert<Input> =
  Input extends EoPropInput<
    infer Type,
    infer Value,
    infer Validator,
    any,
    infer Required
  >
    ? EoPropFinalized<Type, Value, Validator, Input['default'], Required>
    : never;

export {};
